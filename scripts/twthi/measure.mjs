...
import { db } from "../dts2/_db.mjs";

const W = 1080, H = 1948, FROM = 227.8, TO = 286.8, SAFE = 24;

const { data } = await db.from("tracks").select("lyrics").eq("id", "the-world-that-heard-itself").single();
const stamps = [];
...
const probe = async (t) => {
  await p.goto(`http://localhost:3218/studio?track=the-world-that-heard-itself&embed=1&autoplay=1&pass=6&mode=phrase&t=${t}`,
