  ...
  return (
    <>
      {tags.map((t, i) => {
        ...
        return (
          <m.div key={t.id} className="absolute select-none font-display font-black uppercase leading-none"
          ...
          >
            ...
            {!faded && Array.from({ length: 10 }).map((_, k) => {
              ...
              return (
                <m.span key={k} className="absolute rounded-full" aria-hidden
...
function Firework({ word, x, hue }: { word: string; x: number; hue: string }) {
  ...
  return (
    <div className="pointer-events-none absolute inset-y-0" style={{ left: `${x}%` }}>
      ...
      <m.div className="absolute -translate-x-1/2 rounded-full" style={{ width: 8, height: 8, background: hue, boxShadow: `0 0 16px ${hue}` }}
        initial={{ top: "100%", opacity: 1 }} animate={{ top: `${burstY}%`, opacity: [1, 1, 0] }} transition={{ duration: 0.55, ease: "easeOut" }} />
      {/* burst flash */}
      <m.div className="absolute -translate-x-1/2 -translate-y-1/2 rounded-full" style={{ top: `${burstY}%`, width: 20, height: 20, background: "#fff", boxShadow: `0 0 40px 20px ${hue}` }}
      ...
      {Array.from({ length: 18 }).map((_, i) => {
        ...
        return (
          <m.span key={`s${i}`} className="absolute -translate-x-1/2 rounded-full" style={{ top: `${burstY}%`, width: 3, height: 3, background: HUES[i % HUES.length], boxShadow: `0 0 6px currentColor` }}
      ...
      {letters.map((ch, i) => {
        ...
        return (
          <m.span key={i} className="absolute -translate-x-1/2 font-display text-xl font-black uppercase sm:text-2xl"
...
function WhackAWord({ words, idx }: { words: SyncedWord[]; idx: number }) {
  ...
  return (
    <div className="absolute inset-0">
      ...
      {Array.from({ length: HOLES }).map((_, hole) => {
        ...
        return (
          <div key={hole} className="absolute -translate-x-1/2" style={{ left: `${left}%`, top: `${top}%` }}>
            ...
            <AnimatePresence>
              {mole && (
                <m.button key={mole.id} onClick={(e) => whack(mole.id, e)}
      ...
      <AnimatePresence>
        {pops.map((p) => (
          <m.div key={p.id} className="pointer-events-none fixed font-display text-2xl font-black" style={{ left: p.x, top: p.y, color: "var(--theme-primary)" }}
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 px-6">
      ...
      <div className="flex max-w-full flex-wrap items-baseline justify-center gap-x-4">
        {trail.slice(0, -1).map((t, i) => (
          <m.span key={t.id} className="select-none whitespace-nowrap"
      ...
      <div className="relative h-[clamp(4rem,16vw,11rem)]">
        {trail.length > 0 && (
          <m.span key={trail[trail.length - 1].id} className="block select-none whitespace-nowrap"
...
function Downpour({ word, idx }: { word: string; idx: number }) {
  ...
  return (
    <>
      {drops.map((d) => (
        <m.div key={d.id} className="absolute -translate-x-1/2 select-none whitespace-nowrap font-display font-black uppercase"
...
function Bubbles({ word, idx }: { word: string; idx: number }) {
  ...
  return (
    <AnimatePresence>
      {bubbles.map((b) => (
        <m.button key={b.id} onClick={() => pop(b.id)}
...
function Orbit({ word, idx }: { word: string; idx: number }) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      ...
      {orbs.map((o) => (
        <m.div key={o.id} className="absolute left-1/2 top-1/2" style={{ transformOrigin: "0px 0px" }}
...
function Spellcast({ word, idx }: { word: string; idx: number }) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      ...
      <m.div className="absolute h-[52vmin] w-[52vmin] rounded-full border border-dashed"
      ...
      <m.div className="absolute h-[38vmin] w-[38vmin] rounded-full border"
      ...
      <AnimatePresence mode="popLayout">
        {word && (
          <m.span key={idx} className="relative select-none whitespace-nowrap px-4 text-center font-display font-black uppercase"
...
function Aquarium({ word, idx }: { word: string; idx: number }) {
  ...
  return (
    <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 0%, color-mix(in srgb, #123a5c 30%, transparent) 100%)" }}>
      {fish.map((f) => (
        <m.div key={f.id} className="absolute flex items-center gap-1 whitespace-nowrap font-display font-bold uppercase"
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <m.span key={i} className="absolute rounded-full border border-white/25"
  ...
  return (
    <AnimatePresence>
      {puppets.map((p, i) => {
        ...
        return (
          <m.div key={p.id} className="absolute top-0 origin-top" style={{ left: `${p.x}%` }}
    ...
    <div className="absolute inset-0" style={{ perspective: 1100 }}>
      <AnimatePresence>
        {cards.map((c, i) => {
          ...
          return (
            <m.div key={c.id} className="absolute flex h-56 w-36 flex-col items-center justify-center rounded-xl border-2 p-2 text-center sm:h-72 sm:w-44"
  ...
  return (
    <div className="absolute inset-0">
      ...
      {Array.from({ length: 26 }).map((_, i) => (
        <m.span key={`bg${i}`} className="absolute rounded-full bg-white"
      ...
      <AnimatePresence mode="popLayout">
        {word && (
          <m.div key={idx} className="absolute inset-0" initial={{ opacity: 1 }} exit={{ opacity: 0, transition: { duration: 0.5 } }}>
            {/* connecting lines */}
            <svg className="absolute inset-0 h-full w-full">
              {pts.slice(1).map((p, i) => (
                <m.line key={i} x1={`${pts[i].x}%`} y1={`${pts[i].y}%`} x2={`${p.x}%`} y2={`${p.y}%`}
            ...
            {/* the letter-stars */}
            {pts.map((p, i) => (
              <m.span key={i} className="absolute -translate-x-1/2 -translate-y-1/2 select-none font-display font-bold uppercase"
...
function FlapChar({ ch, order, size }: { ch: string; order: number; size: "big" | "small" }) {
  ...
  return (
    <span className={`relative inline-flex items-center justify-center overflow-hidden rounded-[3px] bg-[#14181d] font-mono font-bold ${big ? "h-[1.45em] w-[1em]" : "h-[1.4em] w-[0.95em]"}`}
      ...
      <m.span key={cur} initial={{ rotateX: -75, opacity: 0.4 }} animate={{ rotateX: 0, opacity: 1 }} transition={{ duration: 0.08 }}>
        {cur}
      </m.span>
      {/* the flap seam */}
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 px-4">
      <div className="mb-2 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.4em] text-white/50">
        Departures
        <m.span className="inline-block h-1.5 w-1.5 rounded-full bg-[#ffb63d]" animate={{ opacity: [1, 0.2, 1] }} transition={{ duration: 1.2, repeat: Infinity }} />
      </div>
      {/* settled rows — the board remembers where the song has been */}
      {rows.slice(0, -1).map((r, i) => (
        <m.div key={r.id} className="flex gap-[3px]" style={{ fontSize: "clamp(0.8rem, 2.6vw, 1.3rem)" }}
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-1 p-6 pb-20 sm:p-10 sm:pb-24">
        ...
        {idx >= 0 && word && (
          <div className="lowercase" style={{ color: green, fontSize: "clamp(1.6rem, 6.5vw, 3.4rem)", textShadow: `0 0 12px ${green}, 0 0 30px color-mix(in srgb, ${green} 45%, transparent)` }}>
            &gt; {word.slice(0, typed)}
            <m.span className="ml-1 inline-block h-[0.95em] w-[0.55em] translate-y-[0.12em]" style={{ background: green }}
              animate={{ opacity: [1, 1, 0, 0] }} transition={{ duration: 0.9, repeat: Infinity, times: [0, 0.5, 0.5, 1] }} />
          </div>
        )}
      </div>
      {/* CRT scanlines + a slow flicker */}
      <m.div className="pointer-events-none absolute inset-0"
  ...
  return (
    <div className="absolute inset-0">
      ...
      {Array.from({ length: 5 }).map((_, i) => (
        <m.span key={`f${i}`} className="absolute bottom-[16%] w-4 rounded-t-full"
      ...
      <AnimatePresence>
        {dishes.map((d, i) => {
          ...
          return (
            <m.div key={d.id} className="absolute left-1/2 bottom-[24%] select-none whitespace-nowrap font-display font-black uppercase"
...
function Kaleidoscope({ word, idx }: { word: string; idx: number }) {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      <m.div className="absolute h-2 w-2 rounded-full" style={{ background: hue, boxShadow: `0 0 30px 10px ${hue}` }}
        animate={{ scale: [1, 1.6, 1] }} transition={{ duration: 2.4, repeat: Infinity, ease: "easeInOut" }} />
      <m.div className="absolute left-1/2 top-1/2" animate={{ rotate: 360 }} transition={{ duration: 36, repeat: Infinity, ease: "linear" }}>
        <AnimatePresence mode="popLayout">
          {word && Array.from({ length: SEGS }).map((_, i) => (
            <m.div key={`${idx}-${i}`} className="absolute" style={{ transform: `rotate(${(i * 360) / SEGS}deg)` }}>
              <m.span className="block select-none whitespace-nowrap font-display font-black uppercase"
...
function Ouija({ word, idx }: { word: string; idx: number }) {
  ...
  return (
    <div className="absolute inset-0">
      ...
      {word && seq.map((p, k) => (
        <m.span key={`${idx}-h${k}`} className="absolute -translate-x-1/2 -translate-y-1/2 select-none font-serif"
      ...
      {word && seq.length > 0 && (
        <m.div key={idx} className="absolute z-10 -translate-x-1/2 -translate-y-1/2"
      ...
      <AnimatePresence mode="popLayout">
        {word && (
          <m.div key={idx} className="absolute bottom-[10%] left-1/2 -translate-x-1/2 select-none whitespace-nowrap font-serif uppercase tracking-[0.25em]"
